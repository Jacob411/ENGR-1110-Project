...
InstructionLabel = Label(Header, textvariable= instruction, bg='black', fg='white').pack()
...
def contButtonCreateTeam():
  ...
  continueButtonAddTeamsMembers = Button(Content, text="Add Members", command=lambda:[addTeamsMembers(), continueButtonAddTeamsMembers.grid_remove()])   
  ...
  for i in range(int(numTeamMembers)):
    en = Entry(Content)
    en2 = Entry(Content)
    en.grid(row=i, column=1, ipady = 3)
    en2.grid(row = i, column=2, ipady = 3)
    entries.append(en)
    entries2.append(en2)
...
def contButtonGetSchedule():
  ...
  doneButton = Button(Content, text= "Done", command= lambda: [taskListLabel.grid_remove(), cal.grid_remove(), listRemove(), doneButton.grid_remove()])
...
def contButton1():
  ...
  if selected.get() == options[1]:
    ...
    continueButton = Button(Content, text = "Continue add member", command=lambda: [contButtonAddMember(), continueButton.grid_remove()])
  ...
  if selected.get() == options[3]:
    ...
    continueButton = Button(Content, text="Continue Get schedule", command=lambda: [contButtonGetSchedule(), continueButton.grid_remove()])
    continueButton.grid(row=1, column=0)
  ...
  if selected.get() == options[4]:
      ...
      instruction.set("Enter a Description, Role, and Due Date for the Task")
      cal.grid(row=1, column=2)
      descriptionEntry.grid(row=0, column=1)
      descriptionEntry.insert(0, "Enter description")
      continueButton = Button(Content, text="Continue add task", command=lambda: [contButtonAddTask(), continueButton.grid_remove()])
      continueButton.grid(row=1, column=0)
...
continueButton = Button(Content, text="Continue", command=lambda: [contButtonCreateTeam(), continueButton.grid_remove(), incrementButton.grid_remove(), decrementButton.grid_remove(), numTeamMembersEntry.grid_remove()])
...
def decrement():
  global memCount
  memCount -= 1
incrementButton = Button(Content, text="+", command=lambda: [numTeamMembersEntry.delete(0, END), increment(), numTeamMembersEntry.insert(0, memCount)])
decrementButton = Button(Content, text="-", command=lambda: [numTeamMembersEntry.delete(0, END), decrement(), numTeamMembersEntry.insert(0, memCount)])
...
def startUp():  
  ...
  dropDownButton = Button(Content, text="Select option", command = contButton1).grid(row=2,column=0)
